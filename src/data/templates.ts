...
`,
  ...
  readme: (config: { projectName: string; genre: string; complexity: string; useNewInputSystem: boolean }) => `# ${config.projectName} - Nexus Prime World

Generated by **Unity World Architect v8.0 - Nexus Prime Engine**.
...
${config.complexity === 'Enterprise' || config.complexity === 'UltimatePro' || config.complexity === 'OmniArchitect' || config.complexity === 'NexusPrime' ? '- **ItemDatabase**: ScriptableObject data container.\n- **HighQualitySettings**: Universal Render Pipeline asset.' : ''}
${config.complexity === 'UltimatePro' || config.complexity === 'OmniArchitect' || config.complexity === 'NexusPrime' ? '- **AI/BehaviorTree**: Advanced NPC logic.\n- **AbilitySystem**: ScriptableObject abilities.' : ''}
...
- **Layered Architecture**: Core, Managers, Gameplay, Patterns, AI, UI, Systems, Networking.
${config.complexity === 'Enterprise' || config.complexity === 'UltimatePro' || config.complexity === 'OmniArchitect' || config.complexity === 'NexusPrime' ? '- **Enterprise Patterns**: Service Locator, Object Pooling, Save System, Event Bus.\n- **Localization**: Basic localization management system.' : ''}
${config.complexity === 'UltimatePro' || config.complexity === 'OmniArchitect' || config.complexity === 'NexusPrime' ? '- **Professional Tooling**: Custom Editor inspectors.\n- **Automated QA**: Pre-configured Unit Test suite.' : ''}
